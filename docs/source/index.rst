.. pyquerylist documentation master file, created by
   sphinx-quickstart on Tue Jun  1 19:48:34 2021.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

pyquerylist |version| documentation
===================================

Introduction
------------

Extension of base Python ``list`` that you can query. Queries are chainable and invertable with logical operators (``& | ~``). Simple operations such as ``.where(...)``, to filter, ``.select(...)`` to choose fields, and ``.aggregate(...)`` to e.g. sum over a field are supported. ``.groupby(...)`` can be also used to ``.count()`` and ``.aggregate(...)`` ``QueryList`` as well.
